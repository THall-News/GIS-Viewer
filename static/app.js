// Initialize map with preferCanvas enabled as a baseline performance fallback
const map = L.map('map', { preferCanvas: true }).setView([0, 0], 2);
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19 }).addTo(map);

// Initialize custom preview pane on top of everything and give it a dedicated canvas renderer
...
let fetchedLayers = [];
let activeLayers = [];
let previewLayers = {}; // Track temporary map preview layers
let currentServerUrl = '';
let currentServerType = '';

// Variables for OSM feature splitting
...
// Safely remove a custom pane from the Leaflet map DOM
...
const updateMapLayerOrder = () => {
    // Process layers from bottom of the UI list to the top.
...
const createGeoJsonPointToLayer = (styleState, paneName, customRenderer) => {
    return function(feature, latlng) {
        ...
        if (shape === 'circle') {
            ...
            const w = size * 2 + 4; // Add padding for stroke
            const c = w / 2;
...
const serializeWorkspace = () => {
    ...
    const layersData = activeLayers.map(l => ({
        ...
        isLocalGeoJSON: l.isLocalGeoJSON,
    ...
    return {
        ...
        mapState: { lat: center.lat, lng: center.lng, zoom },
        activeLayers: layersData
    };
};
...
const autoSaveWorkspace = () => {
    try {
        ...
        localStorage.setItem('gis_previewer_auto_save', JSON.stringify(data));
    } catch (e) {
        console.warn("Auto-save failed: Storage limit may be reached.", e);
    }
};
        ...
        data.activeLayers.forEach(lData => {
            ...
            if (mapLayer) {
                ...
                activeLayers.push({
                    ...
                    mapLayer: mapLayer,
                    exportUrl: lData.exportUrl,
                    isLocalGeoJSON: lData.isLocalGeoJSON,
                    geoJsonData: lData.geoJsonData,
                    customStyle: lData.customStyle,
                    isVisible: lData.isVisible ?? true
                });
            }
        });
    }
...
document.getElementById('file-import-workspace').addEventListener('change', (e) => {
    ...
    reader.onload = (evt) => {
        try {
            ...
            console.error(err);
            showToast("Failed to parse workspace JSON file.", true);
        } finally {
            e.target.value = ''; 
        }
    };
...
document.getElementById('btn-clear-workspace').addEventListener('click', () => {
    ...
    if (confirm("Reset workspace? All added layers will be removed from the map.")) {
        ...
        activeLayers = [];
        localStorage.removeItem('gis_previewer_auto_save');
        renderAddedLayers();
...
savedServersSelect.addEventListener('change', (e) => {
    ...
    if (opt.value) {
        ...
        const sType = document.getElementById('server-type');
        sType.value = opt.dataset.type;
...
btnSaveServer.addEventListener('click', async () => {
    ...
    try {
        ...
        if (res.ok) {
            showToast("Server added to Database!");
            await loadSavedServers();
            savedServersSelect.value = url;
        }
    } catch (err) {
        showToast("Failed to save server.", true);
    }
});

const switchTab = (tabName) => {
  if (tabName === 'available') {
    tabBtnAvailable.classList.replace('text-gray-500', 'text-blue-600');
    tabBtnAvailable.classList.replace('dark:text-gray-400', 'dark:text-blue-400');
    tabBtnAvailable.classList.replace('border-transparent', 'border-blue-600');
    tabBtnAvailable.classList.replace('dark:border-transparent', 'dark:border-blue-400');
    
    tabBtnAdded.classList.replace('text-blue-600', 'text-gray-500');
    tabBtnAdded.classList.replace('dark:text-blue-400', 'dark:text-gray-400');
    tabBtnAdded.classList.replace('border-blue-600', 'border-transparent');
    tabBtnAdded.classList.replace('dark:border-blue-400', 'dark:border-transparent');
    
    tabAvailable.classList.replace('hidden', 'flex');
    tabAdded.classList.replace('flex', 'hidden');
    btnAddBulk.classList.remove('hidden');
  } else {
    tabBtnAdded.classList.replace('text-gray-500', 'text-blue-600');
    tabBtnAdded.classList.replace('dark:text-gray-400', 'dark:text-blue-400');
    tabBtnAdded.classList.replace('border-transparent', 'border-blue-600');
    tabBtnAdded.classList.replace('dark:border-transparent', 'dark:border-blue-400');
    
    tabBtnAvailable.classList.replace('text-blue-600', 'text-gray-500');
    tabBtnAvailable.classList.replace('dark:text-blue-400', 'dark:text-gray-400');
    tabBtnAvailable.classList.replace('border-blue-600', 'border-transparent');
    tabBtnAvailable.classList.replace('dark:border-blue-400', 'dark:border-transparent');
...
const closeAllPanels = () => {
    ...
    editPanelContainer.classList.add('hidden');
    editPanelContainer.classList.remove('flex');
    splitPanelContainer.classList.add('hidden');
    splitPanelContainer.classList.remove('flex');
    cropPanelContainer.classList.add('hidden');
    cropPanelContainer.classList.remove('flex');
...
const triggerSearch = () => {
  ...
  if (visibleCount === 0 && fetchedLayers.length > 0) {
    if (!emptyMsg) {
      ...
      emptyMsg.className = 'text-sm text-gray-400 dark:text-gray-500 italic text-center mt-4';
      emptyMsg.textContent = 'No matching layers found.';
...
const triggerAddedSearch = () => {
  ...
  if (visibleCount === 0 && activeLayers.length > 0) {
    if (!emptyMsg) {
      emptyMsg = document.createElement('p');
      emptyMsg.id = 'added-search-empty-msg';
      emptyMsg.className = 'text-sm text-gray-400 dark:text-gray-500 italic text-center mt-4';
      emptyMsg.textContent = 'No matching layers found.';
...
btnClearAddedSearch.addEventListener('click', () => {
  addedLayerSearch.value = '';
  triggerAddedSearch();
  addedLayerSearch.focus();
});
...
const triggerFilterDataSearch = () => {
    ...
    if (term === '') {
        ...
        btnClearFilterSearch.classList.remove('hidden');
    }
...
btnClearFilterSearch.addEventListener('click', () => {
    filterDataSearch.value = '';
    triggerFilterDataSearch();
    filterDataSearch.focus();
});
...
const addLayerToMap = (layerId, switchTabAfter = true) => {
    ...
    if (switchTabAfter) {
        renderAddedLayers();
        switchTab('added');
        showToast(`Added ${meta.title} to map!`);
    }
};
...
const renderAvailableLayers = () => {
  ...
  fetchedLayers.forEach((layer) => {
    ...
    div.innerHTML = `
      ...
      <button class="btn-add-single shrink-0 bg-blue-100 dark:bg-blue-900/40 hover:bg-blue-200 dark:hover:bg-blue-800 text-blue-700 dark:text-blue-300 w-8 h-8 rounded-full flex items-center justify-center transition-colors shadow-sm" data-id="${layer.id}" title="Add Single Layer">
  ...
  document.querySelectorAll('.layer-checkbox').forEach(cb => {
      cb.addEventListener('change', (e) => {
          togglePreviewLayer(e.target.value, e.target.checked);
      });
  });
...
btnAddBulk.addEventListener('click', () => {
  ...
  cbs.forEach(cb => {
    togglePreviewLayer(cb.value, false);
    cb.checked = false;
    addLayerToMap(cb.value, false);
  });
  renderAddedLayers();
  switchTab('added');
  showToast(`Bulk added ${cbs.length} layers to map!`);
});
...
const handleReorder = (e) => {
    ...
    if (action === 'top') {
        ...
        activeLayers.splice(newIdx, 0, layer);
    } else if (action === 'down') {
        const newIdx = Math.min(activeLayers.length, idx + 1);
        activeLayers.splice(newIdx, 0, layer);
...
const handleRename = (e) => {
  ...
  if (newName) { 
      layer.displayName = newName.trim(); 
      renderAddedLayers(); 
      autoSaveWorkspace();
  }
};
  ...
  try {
    ...
    if (features.length === 0) {
      attributeTableContainer.innerHTML = `
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 italic p-2 text-center">No attribute data available for this layer.</p>`;
      return;
    }
    ...
    if(headers.length === 0) {
       attributeTableContainer.innerHTML = `
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 italic p-2 text-center">Features have no text attributes.</p>`;
       return;
    }
    ...
    headers.forEach(h => { 
      tableHtml += `<th class="px-2 py-1 border border-gray-200 dark:border-gray-600 font-semibold text-gray-700 dark:text-gray-200 whitespace-nowrap">${h}</th>`; 
    });
...
const handleToggleEdit = async (e) => {
    ...
    if (features.length > 0 && features[0].properties) {
        cols = Object.keys(features[0].properties);
    }
    ...
    const renderCategoryPickers = () => {
        ...
        let uniqueVals = [...new Set(layer.geoJsonData.features.map(f => f.properties[propName]))];
        uniqueVals = uniqueVals.filter(v => v !== null && v !== undefined);
        
        if (uniqueVals.length > 200 && !confirm(`This column contains ${uniqueVals.length} unique values. Generating color pickers for all of them may slow down your browser. Proceed?`)) {
            return;
        }
        
        if(uniqueVals.length === 0) {
            catInnerList.innerHTML = '<p class="text-xs text-gray-400 dark:text-gray-500 italic text-center">No unique values found.</p>';
            return;
        }
        ...
        uniqueVals.forEach(val => {
            ...
            let sCol = darkenHex(fCol, 0.3); 
            let fOp = 0.5, sOp = 1.0;

            if (isCat && cs.property === propName && cs.categories && cs.categories[val]) {
                fCol = cs.categories[val].fillColor;
                sCol = cs.categories[val].color;
                fOp = cs.categories[val].fillOpacity ?? 0.5;
                sOp = cs.categories[val].opacity ?? 1.0;
            }
    ...
    chkUseData.addEventListener('change', (e) => {
        if(e.target.checked) {
            ...
            catListWrapper.classList.remove('hidden');
            catListWrapper.classList.add('flex');
            ...
            catListWrapper.classList.add('hidden');
            catListWrapper.classList.remove('flex');
    ...
    btnRefreshColors.addEventListener('click', () => {
        document.querySelectorAll('.cat-row').forEach(row => {
            ...
            row.querySelector('.cat-stroke').value = newStroke;
        });
    });

    document.getElementById('btn-copy-style').addEventListener('click', () => {
        if(!layer.customStyle) {
           copiedStyle = { type: 'single', fillColor: '#4f46e5', fillOpacity: 0.5, color: '#4f46e5', opacity: 1.0, pointShape: 'circle', pointSize: 8 };
        } else {
           copiedStyle = JSON.parse(JSON.stringify(layer.customStyle));
        }
    ...
    document.getElementById('btn-apply-edit').addEventListener('click', () => {
        ...
        if (chkUseData.checked) {
            ...
            document.querySelectorAll('.cat-row').forEach(row => {
                ...
                const f = row.querySelector('.cat-fill').value;
                const fOp = parseFloat(row.querySelector('.cat-fill-op').value);
                const s = row.querySelector('.cat-stroke').value;
                const sOp = parseFloat(row.querySelector('.cat-stroke-op').value);
                newCategories[val] = { fillColor: f, fillOpacity: fOp, color: s, opacity: sOp };
            });
    ...
    document.getElementById('btn-bake-colors').addEventListener('click', () => {
        ...
        layer.geoJsonData.features.forEach(f => {
            ...
            let fColor = '#4F46E5', sColor = '#4F46E5', fOp = 0.5, sOp = 1.0;
            if (layer.customStyle.type === 'categorical') {
                const val = f.properties[layer.customStyle.property];
                const cat = layer.customStyle.categories[val];
                ...
                fColor = layer.customStyle.fillColor;
                sColor = layer.customStyle.color;
                fOp = layer.customStyle.fillOpacity;
                sOp = layer.customStyle.opacity;
            }
...
const handleToggleSplit = async (e) => {
    ...
    document.getElementById('btn-apply-split').addEventListener('click', () => {
        ...
        uniqueVals.forEach(val => {
            ...
            const splitStyleState = layer.customStyle 
                ? JSON.parse(JSON.stringify(layer.customStyle)) 
                : { type: 'single', fillColor: '#4f46e5', fillOpacity: 0.5, color: '#4f46e5', opacity: 1.0, pointShape: 'circle', pointSize: 8 };
            ...
            newLayers.push({
                uniqueKey: uniqueKey,
                id: `${layer.id}_${val}`,
                displayName: `${layer.displayName} [${val}]`,
                mapLayer: newMapLayer,
                exportUrl: null, 
                isLocalGeoJSON: true,
                geoJsonData: newGeoJson,
                customStyle: splitStyleState,
                isVisible: true 
            });
            createdCount++;
        });
...
const triggerDataFilterSetup = async (layer) => {
    ...
    drawStatus.textContent = 'Ensuring local data for filtering...';
    filterDataSearch.value = '';
    btnClearFilterSearch.classList.add('hidden');
    
    const success = await ensureGeoJSON(layer);
    if(!success) {
        drawStatus.textContent = 'Failed to load vector data.';
        return;
    }
    ...
    sel.innerHTML = `<option value="" disabled selected>Select attribute column...</option>` + 
        cols.map(c => `<option value="${c}">${c}</option>`).join('');
...
const downloadBlob = (blob, name) => {
  ...
  a.href = url;
  a.download = `${name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.geojson`;
...
document.getElementById('btn-fetch').addEventListener('click', async () => {
  ...
  clearAllPreviews(); 
  document.getElementById('btn-fetch-spinner').classList.remove('hidden');
  document.getElementById('btn-fetch-text').textContent = 'Fetching...';
  ...
  try {
    if (currentServerType === 'OVERPASS') {
        ...
        if (loc) {
            ...
            if (place.osm_type === 'relation') {
                const areaId = 3600000000 + parseInt(place.osm_id);
                query += `area(${areaId})->.searchArea;\n`;
                query += `(\n`;
                if (geomType === 'all' || geomType === 'points') query += `  node${tagFilter}(area.searchArea);\n`;
                if (geomType === 'all' || geomType === 'lines_polygons') {
                    query += `  way${tagFilter}(area.searchArea);\n`;
                    query += `  relation${tagFilter}(area.searchArea);\n`;
                }
                query += `);\n`;
            } else if (place.osm_type === 'way') {
                const areaId = 2400000000 + parseInt(place.osm_id);
                query += `area(${areaId})->.searchArea;\n`;
                query += `(\n`;
                if (geomType === 'all' || geomType === 'points') query += `  node${tagFilter}(area.searchArea);\n`;
                if (geomType === 'all' || geomType === 'lines_polygons') {
                ...
                if (geomType === 'all' || geomType === 'lines_polygons') {
                    query += `  way${tagFilter}(${bbox});\n`;
                    query += `  relation${tagFilter}(${bbox});\n`;
                }
                query += `);\n`;
            }
        } else {
        ...
        geoJson.features.forEach(f => {
            if (f.properties) {
                ...
                delete f.properties.meta;
            }
        });
        
        if (geomType === 'lines_polygons') {
            geoJson.features = geoJson.features.filter(f => !['Point', 'MultiPoint'].includes(f.geometry?.type));
        ...
        if (loc || featName) {
            ...
            } catch(e) { console.error("Could not calculate bounds."); }
        }
        ...
        if (!loc) {
            const cities = {};
            geoJson.features.forEach(f => {
                const c = f.properties['addr:city'] || f.properties['is_in:city'] || f.properties['is_in:municipality'];
        ...
        if (loc && featName) {
            layerName = `OSM: ${featName}, ${loc} (${key})`;
        } else if (featName) {
            layerName = `OSM: ${featName} (${key})`;
        } else if (loc) {
            layerName = `OSM: ${loc} (${key}${val ? '=' + val : ''})`;
        } else if (geoJson.features.length === 1 && geoJson.features[0].properties.name) {
            layerName = `OSM: ${geoJson.features[0].properties.name} (${key}${val ? '=' + val : ''})`;
        } else if (autoCity) {
            layerName = `OSM: ${autoCity} (${key}${val ? '=' + val : ''})`;
        } else {
            layerName = `OSM: Map View (${key}${val ? '=' + val : ''})`;
        }
        ...
        lastFetchedOsmGeoJson = geoJson;
        lastFetchedOsmLayerName = layerName;
        ...
        Array.from(cols).sort().forEach(c => {
            sel.innerHTML += `<option value="${c}">${c}</option>`;
        });

        renderAvailableLayers();
        switchTab('available');
        showToast(`Fetched ${geoJson.features.length} OSM features for preview!`);
        return;
    }
    ...
    if (currentServerType === 'WFS') {
      targetUrl.searchParams.set('service', 'WFS');
    ...
    console.error(e);
    const msg = e.message || "Fetch failed. Check console.";
    showToast(msg, true);
    if (currentServerType !== 'OVERPASS') {
        availableLayerList.innerHTML = `<p class="text-sm text-red-500 dark:text-red-400 italic text-center mt-4">Failed to fetch. Check server URL.</p>`;
        searchContainer.classList.add('hidden');
    }
  } finally {
    document.getElementById('btn-fetch-spinner').classList.add('hidden');
    document.getElementById('btn-fetch-text').textContent = currentServerType === 'OVERPASS' ? 'Fetch OSM Data' : 'Fetch Layers';
  }
});
...
map.on('mousedown', (e) => {
  if (drawingMode === 'box') {
    ...
    drawStart = e.latlng;
    tempShape = L.rectangle([drawStart, drawStart], { color: '#0d9488', weight: 2, fillOpacity: 0.2 }).addTo(drawLayerGroup);
  }
});

map.on('mousemove', (e) => {
  if (drawingMode === 'box' && tempShape) tempShape.setBounds([drawStart, e.latlng]);
});

map.on('mouseup', (e) => {
  if (drawingMode === 'box' && tempShape) {
    map.dragging.enable();
    filterGeometryData = tempShape.getBounds(); 
    drawingMode = null;
    map.getContainer().style.cursor = '';
    checkApplyButton();
  }
...
map.on('click', (e) => {
  if (drawingMode === 'radius') {
    ...
    L.marker(drawStart).addTo(drawLayerGroup);
    filterGeometryData = drawStart; 
    drawingMode = null;
    map.getContainer().style.cursor = '';
    checkApplyButton();
  }
...
btnApplyFilter.addEventListener('click', async () => {
  ...
  if (filterType.value !== 'data' && !targetLayer.exportUrl && !targetLayer.isLocalGeoJSON) {
      return showToast("Cannot filter this layer from server.", true);
  }
  ...
  try {
    ...
    if (filterType.value === 'data') {
        ...
        finalFeatures = targetLayer.geoJsonData.features.filter(f => {
            return selectedVals.includes(String(f.properties[col]));
        });
    } else {
        if (targetLayer.isLocalGeoJSON) {
            ...
            else { const radKm = parseFloat(filterRadius.value) || 5; b = filterGeometryData.toBounds(radKm * 1000); }

            if (queryUrl.includes('WFS') || queryUrl.includes('GetFeature')) {
               queryUrl += `&bbox=${b.getWest()},${b.getSouth()},${b.getEast()},${b.getNorth()},EPSG:4326`;
            } else {
               queryUrl += `&geometry=${b.getWest()},${b.getSouth()},${b.getEast()},${b.getNorth()}&geometryType=esriGeometryEnvelope&inSR=4326`;
            }
            ...
            if (filterType.value === 'radius' && finalFeatures.length > 0) {
                ...
                finalFeatures = finalFeatures.filter(f => {
                    try { return turf.booleanIntersects(f, turfCircle); } catch(e) { return false; } 
                });
            }
        }
    }
    ...
    if (finalFeatures.length === 0) {
      showToast("Filter resulted in 0 features.", true);
      return;
    }
    ...
    activeLayers.unshift({
      uniqueKey: uniqueKey,
      id: `${targetLayer.id}_filtered`,
      displayName: `${namePrefix} ${targetLayer.displayName}`,
      mapLayer: newMapLayer,
      exportUrl: null, 
      isLocalGeoJSON: true,
    ...
    if (targetLayer.isVisible) {
        ...
        map.removeLayer(targetLayer.mapLayer);
    }
    ...
    document.getElementById('btn-filter-text').textContent = 'Apply';
    document.getElementById('btn-filter-spinner').classList.add('hidden');
    btnApplyFilter.disabled = false;
  }
});
...
osmKeyInput.addEventListener('input', (e) => {
    ...
    values.forEach(val => {
        const opt = document.createElement('option');
        opt.value = val;
        osmValueDatalist.appendChild(opt);
    });
});